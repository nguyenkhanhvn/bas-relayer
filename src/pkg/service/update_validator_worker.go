...
func (w *UpdateValidatorWorker) UpdateValidatorSetFromChain(fromChain *UpdateValidatorWorker, id uint64) {
	...
	{
		// fetch block confirmation
		currentValidatorSet, err := w.RelayHub.GetLatestValidatorSet(nil, big.NewInt(fromChain.Config.ChainConfig.ChainId))
		if err != nil {
		...
		fromChain.Config.ChainConfig.SetBlockConfirmations(int64(len(currentValidatorSet))*2/3 + 1)
	}
...
func (w *UpdateValidatorWorker) processUpdateValidatorSet(fromChain *UpdateValidatorWorker, epoch, lastestBlock int64) (bool, error) {
	...
	if toBlock <= lastestBlock {
		blockProofsRaw, blockProofs, err := utils.GenerateBlockProofs(fromChain.Client, int64(fromBlock), int64(toBlock))
		if err != nil {
		...
		tx, errS := w.RelayHub.UpdateValidatorSetUsingEpochBlocks(opts, big.NewInt(fromChain.Config.ChainConfig.ChainId), blockProofs.HeaderRlps)
		if errS != nil {
		...
		if len(blockProofsRaw.Headers) > 0 {
			currentValidatorSet, _ := utils.ParseValidatorSet(blockProofsRaw.Headers[0])
			newBlockConfirmation := int64(len(currentValidatorSet))*2/3 + 1
